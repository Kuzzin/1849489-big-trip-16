...
export const createSitePointTemplate = (point) => {
  const {dateTime, startTime, endTime, eventDuration, pointType, destination, price, offers, isFavorite} = point;

  const date = dateTime !== null
    ? dayjs(dateTime).format('MMM D')
    : '';

  const startEvent = startTime !== null
    ? dayjs(startTime).format('HH:mm')
    : '';

  const endEvent = endTime !== null
